...
class Mamba2(nn.Module):
    ...
    def forward(self, u, H, W, seq_idx=None):
        ...
        assert K in [1, 2, 4, 8]
        if K >= 2:
            xBCdts.append(self.scans.encode(xBCdt.view(B, -1, L)))
        if K >= 4:
            xBCdts.append(self.scans.encode(torch.transpose(xBCdt, dim0=2, dim1=3).contiguous().view(B, -1, L)))
        if K >= 8:
            xBCdts.append(self.scans.encode(torch.rot90(xBCdt, k=1, dims=(2, 3)).contiguous().view(B, -1, L)))
            xBCdts.append(self.scans.encode(
                torch.transpose(torch.rot90(xBCdt, k=1, dims=(2, 3)), dim0=2, dim1=3).contiguous().view(B, -1, L)))

        xBCdts = torch.stack(xBCdts, dim=1).view(B, K // 2, -1, L)
        xBCdts = torch.cat([xBCdts, torch.flip(xBCdts, dims=[-1])], dim=1)
        xBCdts = xBCdts.to(u.dtype)

        xBC, dt = torch.split(
            xBCdts, [self.d_inner + 2 * self.d_state, self.nheads_g], dim=2
        )
        xBC = xBC.view(B, -1, L)
        dt = F.softplus(dt + self.dt_bias).permute(0, 3, 1, 2).flatten(2)  # (B, L, nheads)
        ...
        x, B, C = torch.split(xBC, [self.ngroups * self.d_inner, self.ngroups * self.d_state, self.ngroups * self.d_state], dim=-1)
        ...
        y = torch.sum(y.view(B, L, K, -1), dim=-2)
